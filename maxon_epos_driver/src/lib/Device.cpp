...
{
    RCLCPP_INFO("Called CloseDevice");
    unsigned int error_code;
    if (VCS_CloseDevice(raw_device_ptr, &error_code) == VCS_FALSE) {
        RCLCPP_ERROR_STREAM("CloseDevice (" + EposException::ToErrorInfo(error_code) + ")");
    }
}
...
{
    RCLCPP_INFO("Called CloseSubDevice");
    unsigned int error_code;
    if (VCS_CloseSubDevice(raw_device_ptr, &error_code) == VCS_FALSE) {
        RCLCPP_ERROR_STREAM("CloseSubDevice (" + EposException::ToErrorInfo(error_code) + ")");
    }
}
...
NodeHandle HandleManager::CreateEposHandle(const DeviceInfo &device_info, const unsigned short node_id)
{
    ...
    try {
        ...
        if (existing_handle) {
            RCLCPP_INFO("Found in existing handles");

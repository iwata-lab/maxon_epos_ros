...
void EposProfilePositionMode::init(rclcpp::NodeHandle &motor_nh, NodeHandle &node_handle)
{
    ControlModeBase::init(motor_nh, node_handle);

    if (m_use_ros_unit) {
        rclcpp::NodeHandle encoder_nh(motor_nh, "encoder");
...
{
    int quad_count;
    RCLCPP_DEBUG_STREAM("Target Position: " << position);
    RCLCPP_DEBUG_STREAM("Encoder Resolution: " << m_max_qc);
    if (m_use_ros_unit) {
        quad_count = static_cast<int>((position / (2 * M_PI)) * m_max_qc);
    } else {
        quad_count = static_cast<int>(position);
    }
    RCLCPP_DEBUG_STREAM("Send Quad Count: " << quad_count);
